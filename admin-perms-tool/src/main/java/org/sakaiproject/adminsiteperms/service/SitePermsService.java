...
public class SitePermsService {
    ...
    private static int PAUSE_TIME_MS = ServerConfigurationService.getInt("site-perms-service.pause-time-ms", 1000*2);
    private static long MAX_UPDATE_TIME = Long.valueOf(ServerConfigurationService.getString("site-perms-service.max-update-time-ms", "5400000"));

    private static int NUM_SITES_PER_BATCH = 50;
            ...
            for (Site site : sites) {
                ...
                try {
                    ...
                    if (authzGroupService.allowUpdate(ag.getId())) {
                        ...
                        successCount++;
                        if (updatesCount > 0 && updatesCount % NUM_SITES_PER_BATCH == 0) {
                            // pause every NUM_SITES_PER_BATCH sites updated or so for about 1/2 second
